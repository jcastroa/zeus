...
class Authorization_Token 
{
    ...
    private function tokenIsExistCookie()
    {
      //  $token = $_COOKIE["jwt"];
      //echo $_COOKIE["jwt"];die();
        if (isset($_COOKIE["jwt"]) AND !empty($_COOKIE["jwt"])){
           
            return ['status' => TRUE, 'token' => $_COOKIE["jwt"] ];
        }
        return ['status' => FALSE, 'message' => 'Token is not defined.'];
    }


    public function tokenCsrfIsExist()
    {
        $headers = $this->CI->input->request_headers();
        if(!empty($headers) AND is_array($headers)) {
            foreach ($headers as $header_name => $header_value) {
                if (strtolower(trim($header_name)) == strtolower(trim('X-CSRF-TOKEN')))
                    return ['status' => TRUE, 'token-csrf' => $header_value];
            }
        }
        return ['status' => FALSE, 'message' => 'Token csrf is not defined.'];
    }



    public function validateTokenCookie()
    {
        $token_data = $this->tokenIsExistCookie();
        if($token_data['status'] === TRUE)
        {
            try
            {
